// student_missed/lib/src/commands/discipline_commands.dart
...
class DisciplineCommands extends BaseCommands<DisciplineModel> {
  ...
  Future<void> assignToGroup() async {
    ...
    final groups =
    await db.groupDao.getGroupsWithoutDiscipline(discipline.id);
    ...
    try {
      ...
      ConsoleHelpers.printSuccess(
        'Дисциплина "${discipline.name}" привязана '
            'к группе "${group.name}"',
      );
    } catch (e) {
      ConsoleHelpers.printError('Не удалось привязать '
          'дисциплину: $e');
    }
  }
}
