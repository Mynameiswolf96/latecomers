...
class DisciplineDao implements IDisciplineDao {
  ...
  Future<bool> exists(String name) async{
    final List<Map<String,dynamic>> maps=await _db.query('disciplines',where: 'name = ?',whereArgs: [name]);
  ...
  Future<List<GroupModel>> getAssignedGroups(int disciplineId) async{
    final List<Map<String,dynamic>> maps=await _db.rawQuery(''
        ...
        '',[disciplineId]);
    return List.generate(maps.length, (i){
      return GroupModel(id: maps[i]['id']as int, name:maps[i]['name']as String);
    });
  }
